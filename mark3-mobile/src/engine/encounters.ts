...
const KINDS: Kind[] = [
  {
    ...
    // 넉넉하고 정의로운 나라는 돕는다. 평판이 무너진 나라도 여유가 있으면 돕는다 —
    // 평판을 되살릴 몇 안 되는 길이다.
    pick: (n) => ((n.gold > 150 && n.justice > 65) || (n.gold > 60 && n.justice < 40) ? 1 : 0),
  },
  {
    kind: 'merc-join',
  ...
  },
  {
    ...
    // 평판이 바닥이면 돌려준다 (아래 conscript 주석 참고)
    pick: (n) => (n.justice > 70 || n.justice < 40 ? 1 : 0),
  },
  {
  ...
  },
  {
    ...
    /*
      처음엔 '정의 50 미만이면 끌고 간다' 였다. 끌고 가면 정의가 더 깎이니,
      한 번 50 아래로 내려간 나라는 끝없이 잔혹해졌다 — 21x21 에서 판 끝 정의
      평균이 43.7 → 29.5, 정의 35 미만 나라가 13% → 51%. 공포가 높으면 항복한
      병사의 처우가 가혹해져 끝까지 버티는 전쟁이 되고, 판이 늘어졌다.
      평판에 여유가 있을 때만 끌고 간다.
    */
    pick: (n) => (n.justice >= 45 ? 0 : 1),
  },
  {
    kind: 'poisoned-well',
